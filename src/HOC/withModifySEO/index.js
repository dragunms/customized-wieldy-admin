...
import {Card, Checkbox, Form, Tabs, Input, Divider} from 'antd';
...
import SEOTab from 'src/partials/SEOTab';
...
const withModifySEO = (
    ...
    const HookModifySEO = (props) => {
        ...
        const tabs = [
            ...
            {
                title: <TranslationText id='label.seo_tab' />,
                content: (
                    <Tabs.TabPane tab={<TranslationText id='label.seo_tab' />} key='2' forceRender>
                        <SEOTab formControl={formControl} data={data} />
                    </Tabs.TabPane>
                ),
            },
        ];
        return (
            <>
                ...
                {loading ? (
                    ...
                    <AvForm defaultValues={data} form={formControl} onSubmit={handleOnSubmit}>
                        <Tabs defaultActiveKey='1'>
                            {tabs.map((tabPane, index) => {
                                return <React.Fragment key={index.toString()}>{tabPane.content}</React.Fragment>;
                            })}
                        </Tabs>
    ...
    return withUser(roles.roles)(HookModifySEO);
};
export default withModifySEO;
